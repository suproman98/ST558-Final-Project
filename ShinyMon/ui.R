...
navbarPage(
    ...
    tabPanel("Data Exploration",
             sidebarLayout(
                 sidebarPanel(
                   selectInput('attr1', 'Inform first attribute (X-axis)', choices = c("hp", "speed", "attack", "defense", "sp_attack", "sp_defense")),
                   selectInput('attr2', 'Inform second attribute (Y-axis)', choices = c("hp", "speed", "attack", "defense", "sp_attack", "sp_defense"), selected = "attack"),
                   sliderInput('gen', 'Starting from generation:', min = 1, max = 8, value = 1, step = 1)
                   ),
                 mainPanel(strong(uiOutput("title", align = "center")), plotOutput("plot"))
             )),
    tabPanel("Modeling", mainPanel(dataTableOutput("table")))
)